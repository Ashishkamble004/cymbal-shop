...
// Configure this URL after deploying the tata-neu-server to Cloud Run
// Format: wss://tata-neu-server-<PROJECT_NUMBER>.<REGION>.run.app
const TATA_NEU_SERVER_URL = 'wss://tata-neu-server-769002985772.us-central1.run.app';

class AudioClient {
    constructor(serverUrl = TATA_NEU_SERVER_URL) {
        this.serverBaseUrl = serverUrl;
        this.serverUrl = serverUrl; // Will be updated with full path in connect()
        this.ws = null;
        this.recorder = null;
        ...
        this.onInputTranscription = (text, finished) => {};
        this.onOutputTranscription = (text, finished) => {};
    ...
    async connect() {
        ...
        // Generate user_id and session_id if not set
        if (!this.userId) {
            this.userId = 'user-' + this.generateId();
        }
        if (!this.sessionId) {
            this.sessionId = 'session-' + this.generateId();
        }

        // Build full WebSocket URL with path
        this.serverUrl = `${this.serverBaseUrl}/ws/${this.userId}/${this.sessionId}`;
                ...
                this.ws.onmessage = async (event) => {
                    try {
                        ...
                        if (message.type === 'ready') {
                            ...
                            console.log('Input transcription:', message.text, 'finished:', message.finished);
                            this.onInputTranscription(message.text, message.finished);
                        }
                        else if (message.type === 'output_transcription') {
                            // Handle model's speech transcription
                            console.log('Output transcription:', message.text, 'finished:', message.finished);
                            this.onOutputTranscription(message.text, message.finished);
                        }
                        else if (message.type === 'tool_call') {
                            // Handle tool/agent calls
                            console.log('Tool call:', message.data);
                            this.onToolCall(message.data.name, message.data.args);
                        }
    ...
    sendVideo(base64Data) {
        if (!this.isConnected || !this.ws || this.ws.readyState !== WebSocket.OPEN) {
            console.warn('Cannot send video: not connected');
            return false;
        }
